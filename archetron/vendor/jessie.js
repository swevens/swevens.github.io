var jessie = {
    shouldIBeLookingForNames: false
};
...
(function(){
        ...
        for (var i=event.resultIndex; i<event.results.length; i++){
            ...
            if (event.results[i].isFinal){
                ...
                transcript.split(' ').forEach(function(word){
                    if (jessie.shouldIBeLookingForNames && jessie.commands.close.indexOf(word) !== -1)
                        commandMeta.close.callback();
                    else if (!jessie.shouldIBeLookingForNames && jessie.commands.search.indexOf(word) !== -1)
    ...
    jessie.onClose = function(callback){
        registerCommandCallback('close', function(){
            jessie.shouldIBeLookingForNames = false;
            callback();
        });
        return this;
    };
    ...
    jessie.onSearch = function(callback){
        registerCommandCallback('search', function(){
            jessie.shouldIBeLookingForNames = true;
            callback();
        });
        return this;
    };
